...
export default function LecturesPage() {
  ...
  }, [selectedLecture, isFullscreen, handleToggleFullscreen]);
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      >("echo360_sync_lectures", { canvasCourseId: selectedSubjectId });
  ...
  const handleSelectLecture = async (lec: Lecture) => {
    ...
    if (lec.transcript_path) {
      try {
        const vtt = await invoke<string>("echo360_read_transcript", {
          path: lec.transcript_path,
        });
        setCues(parseVtt(vtt));
      } catch {
    ...
    if (lec.video_path && lec.progress_seconds > 5) {
      const onLoaded = () => {
        if (videoRef.current)
          videoRef.current.currentTime = lec.progress_seconds;
  ...
  const handleTimeUpdate = () => {
    ...
    progressSaveRef.current = setTimeout(async () => {
      ...
      if (
        selectedLecture.duration_seconds > 0 &&
        v.currentTime >= selectedLecture.duration_seconds - 30
      ) {
          ...
          <>
            ...
            <div className="flex-1 bg-black flex flex-col min-h-0 relative overflow-hidden">
              {videoSrc ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-4 text-white/60 p-6">
                  ...
                  <p className="text-xs">
                    {fmtDate(selectedLecture.date)} ·{" "}
                    {fmtDuration(selectedLecture.duration_seconds)}
                  </p>
            ...
            <div className="h-10 shrink-0 bg-card border-t border-border flex items-center px-3 gap-3">
              ...
              <span className="text-[11px] text-muted-foreground font-mono min-w-[80px]">
                ...
                {fmtTime(selectedLecture.duration_seconds)}
              </span>

              {/* Scrub bar */}
              <div
                className="flex-1 h-1.5 bg-surface-raised rounded-full overflow-hidden cursor-pointer"
                onClick={(e) => {
                  const rect = e.currentTarget.getBoundingClientRect();
                  const pct = (e.clientX - rect.left) / rect.width;
                  if (videoRef.current)
                    videoRef.current.currentTime =
                      pct * selectedLecture.duration_seconds;
                }}
              >
                <div
                  className="h-full bg-primary rounded-full transition-none"
                  style={{
                    width: `${selectedLecture.duration_seconds ? (currentTime / selectedLecture.duration_seconds) * 100 : 0}%`,
                  }}
                />
              </div>
              ...
              <div className="flex items-center gap-1 ml-1">
                ...
                <button
                  onClick={() => {
                    if (selectedLecture.transcript_path) {
                      if (cues.length > 0) {
                      ...
                      handleDownloadTranscript(selectedLecture);
                    }
                  }}
                  title={
                ...
                <button
                  ...
                  disabled={!selectedLecture.transcript_path}
                  title={captionsEnabled ? "Hide captions" : "Show captions"}
                  className={cn(
                    "p-1 rounded hover:bg-surface transition-colors disabled:opacity-30",
                    captionsEnabled
                      ? "text-foreground"
                      : "text-muted-foreground hover:text-foreground",
                  )}
