...
//!
//! It exists for the tab shortcuts. macOS gives the menu bar first refusal
//! on every ⌘-key, before the key reaches any webview, so ⌘W and ⌘T have to
//! be menu items: a `keydown` listener in the frontend never sees them, and
//! while a browser tab's native page holds focus (`browser.rs`) the app's
//! own webview sees no keys at all. Tauri's default menu spends ⌘W on Close
//! Window, which a tabbed window wants for the tab — so the whole menu is
//! built here instead, with Close Window moved to ⇧⌘W.
//!
//! The tab items only emit; the strip owns what a tab is
//! (`app/src/components/tabs/TopTabBar.tsx`).
